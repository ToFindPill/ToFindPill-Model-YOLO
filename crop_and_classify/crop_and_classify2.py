...
def classify_image(image_path, model_path):
    ...
    for result in predictions:
        ...
        # ' copy'를 제거
        if ' copy' in class_name:
            class_name = class_name.replace(' copy', '')

        print(f'\n이미지 경로: {result.path}')
        
        if top1_class_id in valid_classes and top1_confidence > 0:
            print(f"예측된 Top-1 클래스: {class_name}, 신뢰도: {top1_confidence:.4f}")
        else:
            print(f"오류: 예측된 클래스 {top1_class_id}가 유효한 범위를 벗어났거나 신뢰도가 0입니다.")
        ...
        for i, class_id in enumerate(top5_classes):
            ...
            if class_id in valid_classes and confidence >= 0.0001:
                print(f"클래스: {class_name}, 신뢰도: {confidence:.4f}")
